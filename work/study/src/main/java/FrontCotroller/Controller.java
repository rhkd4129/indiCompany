...
import commonUtils.ExceptionHandler;
import commonUtils.FileUtil;

import servletUtils.MyView;
...
@WebServlet(name = "FrontController", urlPatterns = "*.do", initParams = {
		@WebInitParam(name = "config", value = "/WEB-INF/config.json"),
		@WebInitParam(name = "command", value = "/WEB-INF/command.json") })

@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 1 MB
...
public class Controller extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			/*
			 * request된 URL를 분석하여 파싱후에 classNames변수에는 해당 서비스 methodName에는 해당 메서드가 저장 EX)
			 * "/view/board/list.do"
			 */

			String comObject = comArr[1]; // board
			String comAction = comArr[2]; // list.do
			String classNames = "service." + comObject.substring(0, 1).toUpperCase() + comObject.substring(1)
					+ "Service";
			...
			Map<String, Object> model = ServletRequestMapper.invokeController(classNames, methodName, paramMap);
			logger.info("2:{}", model);

			/*
			 * request에서 파싱해서 온 클래스와 메서드 이름 파라미터를 넘겨 model을 반환 각각의 기능에 맞게 view.render호출 ->
			 * view로 forward하거나 json를 전송하는 함수
			 */
